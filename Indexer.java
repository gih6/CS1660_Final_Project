...
public class Indexer{
    ...
    public ArrayList<Document> searchForTerm(String term){
    	// System.out.println("These are document folder: "); 
    	ArrayList<Document> arList = new ArrayList<>();
    	if(storage.containsKey(term)){
            Word temp2 = storage.get(term);
            arList = temp2.getDocuments();
    ...
    public ArrayList<Word> getMaxFrequencies(int N){
    	ArrayList<Word> arList = new ArrayList<>();
    	for(int i=0;i<N;i++){
             arList.add(listOfValues.get(i));
         }
         return arList;
